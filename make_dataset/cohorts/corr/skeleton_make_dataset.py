...
Pipeline to create skeleton dataset for the CoRR study
...
# Directories
neurospin = "/neurospin"
study_dir = os.path.join(neurospin, "psy_sbox", "CoRR")
...
output_dir = os.path.join(path_to_data, "skeletons", study.lower())
...
path_to_morpho_df = os.path.join(output_dir, "metadata", f"{study.lower()}_morphologist_summary.tsv")
...
morphologist_qc = pd.read_csv(os.path.join(output_dir, "metadata", f"{study.lower()}_morphologist_qc.tsv"), sep="\t", dtype=ID_TYPES)
...
path2save = os.path.join(output_dir, "metadata", f"{study.lower()}_skeleton_qc.tsv")
...
qc_skel_filename = os.path.join(output_dir, "metadata", f"{study.lower()}_skeleton_qc.tsv")
