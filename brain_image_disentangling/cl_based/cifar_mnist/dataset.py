...
class SimCLRDataAugmentation(nn.Module):
    ...
    if __name__ == "__main__":
        ...
        train, test = get_dataloaders(weak_modality="mnist", n_train=50, n_test=10, batch_size=10)

        for weak_img, strong_img, weak_label, strong_label in train:
            print(weak_img.size(), strong_img.size(), weak_label.size, strong_label.size)
            break
