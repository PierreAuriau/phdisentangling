...
class MorphoMNISTDataset(Dataset):
    def __init__(self, split, targets=None):
        ...
        if split == "test":
            split = "t10k"
        self.skeletons = np.load(os.path.join(config.path_to_data, f"{split}-skeletons.npy"), mmap_mode="r")
        self.images = np.load(os.path.join(config.path_to_data, f"{split}-images.npy"), mmap_mode="r")
        self.labels = pd.read_csv(os.path.join(config.path_to_data, f"{split}-morpho.csv"), sep=",")        
