        ...
        for epoch in list_epochs:
            ...
            if os.path.exists(filename):
                ...
                for split in splits:
                    ...
                    if dataset == "hcp":
                        y_true[split] = manager.dataset[split].target.values
                    else:
                ...
                for split in splits:
                    ...
                    if split == "test_intra":
                        ...
                        representations[split], _, y_true[split] = self.get_embeddings(loader.test)
                    else:
                        representations[split], _, y_true[split] = self.get_embeddings(getattr(loader, split))
            ...
            for i, label  in enumerate(labels):
                if label in ["diagnosis", "sex"]:
                    ...
                    metrics = {"rmse": lambda y_pred, y_true: mean_squared_error(y_pred=y_pred, y_true=y_true, squared=False),
                               "mae": lambda y_pred, y_true: mean_absolute_error(y_pred=y_pred, y_true=y_true)}                
                elif label in ["site"]:
