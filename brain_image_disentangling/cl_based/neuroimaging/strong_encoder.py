...
class StrongEncoder(object):
    ...
    def train(self, chkpt_dir, exp_name, dataset, ponderation, nb_epochs, 
        ...
        # loader
        strong_modality = "quasi_raw" if dataset =="hcp" else "vbm"
        manager = TwoModalityDataManager(root="/neurospin/psy_sbox/analyses/2023_pauriau_sepmod/data/root", 
                                         db=dataset, weak_modality="skeleton", strong_modality=strong_modality,
                                         labels=None, batch_size=16, two_views=True,
    ...
    def loss_fn(self, weak_head, common_head, specific_head):
        ...
        self.history.log(common_align_loss=common_align_loss.item(), common_uniform_loss=common_uniform_loss.item())
        ...
        self.history.log(specific_align_loss=specific_align_loss.item(), specific_uniform_loss=specific_uniform_loss.item())
    ...
    def test(self, chkpt_dir, exp_name, dataset, labels, list_epochs):
        ...
        strong_modality = "quasi_raw" if dataset =="hcp" else "vbm"
        manager = TwoModalityDataManager(root="/neurospin/psy_sbox/analyses/2023_pauriau_sepmod/data/root", 
                                           db=dataset, strong_modality=strong_modality, weak_modality="skeleton", 
                                           labels=labels, batch_size=16, two_views=False,
                                           num_workers=8, pin_memory=True)
        self.exp_name = exp_name
        self.chkpdt_dir = chkpt_dir
        ...
        for epoch in list_epochs:
            ...
            for e, v in representations.items():
                for s, w in v.items(): 
                    logger.debug(f"{e} - {s} - {w.shape}")
                    logger.debug(f"y_true - {y_true[s].shape}")
            ...
            for i, label in enumerate(labels):
                ...
                for encoder in ("weak", "common", "specific", "strong"):
                    ...
                    for split in splits:
                        ...
                        values = {}
                        logger.debug(f"y_pred - {y_pred.shape} - y_true - {y_true[split][:, i].shape}")
...
def parse_args(argv):
    ...
    parser.add_argument("--dataset", required=True, type=str, choices=["asd", "bd", "scz", "hcp"],
