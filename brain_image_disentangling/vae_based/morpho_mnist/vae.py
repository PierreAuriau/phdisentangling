...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim=256, nb_layers=None):
        super().__init__()
        if nb_layers == 6:
    ...
    def loss_fn(self, inputs, outputs, mean, logvar):
        ...
        if self.reconstruction_loss == "bce":
            rec_loss = F.binary_cross_entropy_with_logits(outputs, inputs, reduction="sum")
    ...
    def set_reconstruction_loss(self, reconstruction_loss):
