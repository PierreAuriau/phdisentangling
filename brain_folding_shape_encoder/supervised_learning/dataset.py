...
class ClinicalDataset(Dataset):
    ...
    def __init__(self, dataset: str, split: str = 'train', 
        ...
        self.metadata = pd.concat([pd.read_csv(os.path.join(config.path_to_data, f"{s}_t1mri_skeleton_participants.csv")) 
                                   for s in self._studies],
                       ignore_index=True, sort=False)
        self.scheme = self.load_scheme()
        if smooth_preproc:
            self.images = [np.load(os.path.join(config.path_to_data, f"{s}_t1mri_smooth_skeleton_data32.npy"), mmap_mode='r')
                        for s in self._studies]
        else:
            self.images = [np.load(os.path.join(config.path_to_data, f"{s}_t1mri_skeleton_data32.npy"), mmap_mode='r')
                        for s in self._studies]
