...
from dl_training.transforms import Crop, Padding, Normalize, Binarize, GaussianConvolution
...
def load_dl_model(pths, apply_softmax=True):
    ...
    for pth in pths:
        ...
        if "AlexNet" in pth or "alexnet" in pth:
            net = AlexNet3D_Dropout(num_classes=1)
        elif "DenseNet" in pth or "densenet" in pth:
            net = densenet121(num_classes=1)
        elif "ResNet" in pth or "resnet" in pth:
            net = resnet18(in_channels=1, num_classes=1)
        elif "SFCN" in pth or 'sfcn' in pth:
            net = SFCN(num_classes=1, dropout=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--preproc", choices=["vbm", "quasi_raw", "skeleton"], default="vbm")
    ...
    root = args.root
    brain_atlas = (os.path.join(root, "atlas", "AAL3v1_1mm.nii.gz"),
                   os.path.join(root, "atlas", "AAL3v1_1mm.nii.txt"))
    ...
    if args.dl:
        ...
        if preproc in ["vbm", "quasi_raw"]:
            ...
            print("Warning : No Gaussian Convolution")
        else:
        ...
        batch_size = 16
        drop_last = False
        if len(test_dataset) % batch_size == 1 or len(test_intra_dataset) % batch_size == 1:
            drop_last = True
            logger.warning(f"Last batch will not be tested. Change batch size ({batch_size}).")
        test = DataLoader(test_dataset, batch_size=batch_size, num_workers=4, sampler=SequentialSampler(test_dataset),
                          collate_fn=OpenBHBDataManager.collate_fn, drop_last=drop_last)
        test_intra = DataLoader(test_intra_dataset, batch_size=batch_size, num_workers=4,
    ...
    if args.dl:
        assert isinstance(model, torch.nn.Module), "Unknown Torch model %s" % type(model).__name__
    else:
        assert is_regressor(model) or is_classifier(model), "Unknown sklearn model %s" % type(model).__name__
    # Puts the model in evaluation mode (no dropout, batchnorm, etc.)
    model.eval()

    saliency_maps = dict(inter=[], intra=[])
    test_names = ["intra", "inter"]
    for t, t_name in zip([test_intra, test], test_names):
        bar = tqdm(desc="Test %s" % t_name, total=len(t))
            ...
            if args.dl:
                if args.saliency_meth == "occ":
                    ...
                    try:
                        ...
                        logger.warning("All gradients are zeros for batch %i (test %s)" % (i, t_name))
                        exit(0)
                    saliency_maps[t_name].extend(saliency_imgs.astype(np.float32))
    ...
    if args.mask is not None:
        ...
        for t_name in test_names:
            logger.info(f"Test {t_name}"
                        f"Most Important Regions: "
                        f"{get_relevance_per_area(area_masks, np.mean(saliency_maps[t_name], axis=0))[:10]}")
    ...
    dict_to_save = dict(saliency_map={t_name: {'avg': np.mean(saliency_maps[t_name], axis=0),
                                               'std': np.std(saliency_maps[t_name], axis=0)}
                                      for t_name in test_names})
    if args.mask is not None:
        for t_name in test_names:
            dict_to_save["saliency_map"][t_name]['relevances'] = [get_relevance_per_area(area_masks, s_map)
                                                              for s_map in saliency_maps[t_name]]

            dict_to_save["area_masks"] = area_masks
    MLTrainer.save(dict_to_save, saving_file)
    for t_name in test_names:
        np.save(os.path.join(os.path.dirname(chk), f"Test_{t_name}_saliency_maps_{args.saliency_meth}"
                                                   f"_{os.path.basename(os.path.splitext(chk)[0])}.npy"),
                saliency_maps[t_name])
    logger.warning(f"Saliency maps saved at {saving_file}")
