...
if __name__ == "__main__":
    ...
    parser.add_argument("--preproc", required=True, choices=["quasi_raw", "vbm", "skeleton"])
    ...
    parser.add_argument("--input_transforms", nargs="+", choices=["Padding", "Crop", "Normalize", "Standardize",
                                                                  "Binarize", "GaussianConvolution", "RandomRotation"])
    ...
    try:
        ...
        m_skeleton = np.load(os.path.join(root, "brain_mask_dilation_9.npy"))
    ...
    masks = {"vbm": m_vbm.get_data() != 0, "quasi_raw": m_quasi_raw.get_data() != 0, "skeleton": m_skeleton}
    ...
    if args.input_transforms is not None:
        input_transforms = Compose([eval(t) for t in args.input_transforms])
    else:
        input_transforms = None

    val = dataset_cls(root, preproc=preproc, split="val", target=label, transforms=input_transforms)
    test = dataset_cls(root, preproc=preproc, split="test", target=label, transforms=input_transforms)
    test_intra = dataset_cls(root, preproc=preproc, split="test_intra", target=label, transforms=input_transforms)
    ...
    for folds_job_i in folds_per_job:
        ...
        for fold in folds_job_i:
            if N_train is None:
                # fold is ignored currently
                train = dataset_cls(root, preproc=preproc, split="train", target=label,
